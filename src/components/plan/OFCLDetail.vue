    ...
    <div class="plan-detail__content">
      <div class="content-item goods-item">
        ...
        <div class="content-item-detail">
          ...
          <div class="goods-detail__intro warn">
            装箱方案:
            <template v-for="(value, key) in boxInfo">
              <span :key='key'>{{BOXS[key].name | uppercase}} * {{value}} </span>,
            </template>
            剩余空间:
            {{allWeightAndVolume.volume - usedWeightAndVolume.volume}}m³
            ·
            {{Math.floor(allWeightAndVolume.weight - usedWeightAndVolume.weight)}}tons</div>
        </div>
        ...
        <div class="content-item-detail">
          <div class="line-item__intro">
            <el-tooltip placement="top">
              ...
              <p class="ellipsis">
                提货地址：{{`${startAddress.title}(${startAddress.address})`}}
              </p>
            </el-tooltip>
            <p class="ellipsis">
              始发港：{{ $t(`country.${transportRoute.polCountry}`) | capitalize}}
              ·
              {{ $t(`port.${transportRoute.pol}`) | capitalize}}
            </p>
          </div>
          <div class="line-item__intro">
            <el-tooltip placement="top">
              ...
              <p class="ellipsis">
                送货地址：{{`${endAddress.title}(${endAddress.address})`}}
              </p>
            </el-tooltip>
            <p class="ellipsis">
              目的港：{{$t(`country.${transportRoute.podCountry}`) | capitalize}}
              ·
              {{$t(`port.${transportRoute.pod}`) | capitalize}}
            </p>
          </div>
          <div class="line-item__intro">
            <p class="ellipsis">
              船期：每{{info['etd'] | weekday}}开船，提前 4 天提货集装箱
              </p>
            <p>
              全程耗时：{{ parseInt(info['transittime'] || 0) + 7 }} 天
            </p>
          </div>
          <div class="line-item__intro">
            <p>运输方式：{{TRANSPACKTYPES[transportRoute.transPackType]}}·直达</p>
      ...
      <div class="content-item fee-item">
        ...
        <div class="content-item-detail">
          ...
          <div class="fee-item__column">
            <p class="fee-item__header">运输费用<span class="tip">({{info.currency | uppercase}})</span></p>
             <p v-for="(value, id) in boxInfo" :key="id">
              {{BOXS[id].name | uppercase}} x {{value}} = {{partDetail3.chargeDetail.ocf[BOXS[id].name]}}
            </p>
            <p>
              <span class="em">价格有效期</span>
              <br>
              <span class="warn">{{info.rvf}}-{{info.rve}}</span>
            </p>
          </div>
          <div class="fee-item__separator"></div>
          <div class="fee-item__column">
...
export default {
  props: ['data', 'goods', 'query', 'ordered'],
  data () {
    return {}
  },
  ...
  computed: {
    ...
    boxInfo () {
      return JSON.parse(this.data.packing.boxInfo)
    },
    allWeightAndVolume () {
      let allWeight = 0
      let allVolume = 0
      for (let key in this.boxInfo) {
        allWeight += this.BOXS[key].weightlimit * this.boxInfo[key]
        allVolume += this.BOXS[key].volumelimit * this.boxInfo[key] 
      }
      return {
        volume: allVolume,
        weight: allWeight / 1000
      }
    },
    usedWeightAndVolume () {
      let usedWeight = 0
    ...
    ShowPart1Info () {
      const config = this.CHARGEDETAIL['FCL']
