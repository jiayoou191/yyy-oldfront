    ...
    <div class="details">
      <el-form ref="rules" :model="form" label-width="80px" :rules="rules" :status-icon="true">
        ...
        <el-form-item>
          ...
          <el-button v-if="!isEnglish" type="primary" @click="handlePhoneRegister">立即注册(手机)</el-button>
          <el-button v-if="isEnglish" type="primary" @click="handleEmailRegister">立即注册(邮箱)</el-button>
...
export default {
  ...
  methods: {
    ...
    handlePhoneRegister() {
      ...
      httpPostFormData("/register/via/phone", formData).then(res => {
        console.log(res.data);
        // this.$message("注册成功");
        console.log('re form',this.form);
        this.$router.push({name:'register',params:this.form})
        // this.form.phone = "";
        // this.form.email = "";
        // this.form.code = "";
      });
    ...
    handleEmailRegister() {
      ...
      formData.set("email", this.form.email);
      formData.set("code", this.form.code);
      httpPostFormData("/register/via/email", formData).then(res => {
        console.log(res.data);
        // this.$message("注册成功");
        this.$router.push({name:'register',params:this.form})
        this.form.phone = "";
